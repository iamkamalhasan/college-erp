...
const OnDuty = () => {
    ...
    useEffect(() => {
        if(dateFlag)
            axios.get('/fa/attendance', { params: { date: apiDateFormat, batch:batch, section:section, branch:branch } } )
            .then(response => {
    ...
    useEffect(()=> {
        if(saveFlag){
            axios.post('/fa/attendance', tableData )
            .then(response => {
    ...
    const omit = ["_id", "freezed","section", "studentId", "present","marked", "onduty", "masterTimetableId", "courseId", "courseCode", "branch", "batch", "date", "period", "__v", "createdAt", "updatedAt", "courseName"]
