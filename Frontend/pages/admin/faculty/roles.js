    ...
    return ( data &&
        ...
        <div className="max-w-min max-h-[80%] overflow-auto overscroll-none mr-2 rounded-b-lg shadow-md align-middle border rounded-t-lg ">
            <table className="table-auto divide-y divide-gray-200 text-sm text-left">
                ...
                {
                    data.map((row, ridx) => (
                    <tr className={`px-6 py-3 text-sm font-medium text-gray-800 whitespace-nowrap group hover:bg-sky-50`} key={ridx}>
                        <td className="px-6 py-3 text-sm text-gray-800 whitespace-nowrap">{ridx + 1}</td>
                        {
                            fields.map((key, kidx) => edit[ridx] != 1 ? (
                            <td className="px-6 py-3 text-sm text-gray-800 whitespace-nowrap" key={kidx}>
                            { row[key]===true?<Icon name="check"/>:row[key] }
                            </td>) : (
...
const Roles = () => {
    ...
    const omit = ["_id","email","type","firstName","lastName","mobile","primaryRole","title", "isActive", "requestId", "isCredentialCreated", "address"];
    const omitFields = (field) => !omit.some(item => item == field)
