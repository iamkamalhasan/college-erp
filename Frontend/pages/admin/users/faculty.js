...
const Faculty = () => {
    ...
    let omit = [ '_id', 'isCredentialCreated', 'canRequest' ];
    ...
    const filterCheck = (doc) => {
        return (branch == 'ALL' ? true : doc.branch == branch) &&
    ...
    const manageLatest = (obj, value, all = false) => {
        ...
        if(all) {
            if(value || latest.length==0)
                update(data.map(doc => ({ _id: doc._id, isActive: value, email: doc.email, userType: "Faculty" })))
            else update([])
        ...
        if(length==latest.length) update([...latest, { _id: obj._id, isActive: value, email: obj.email, userType: "Faculty" }])
