        ...
        <div className="flex group">
            <div className="border px-2 py-1 rounded-l-lg group-hover:border-blue-500">
                <div className="flex mt-2 space-x-2 cursor-pointer text-sm" onClick={() => setOpen(!open)}>
                    <div className="pt-0.5 lowercase">{ filter }</div>
                ...
                <ul className={`absolute max-h-64 overflow-auto overscroll-none z-10 bg-white rounded shadow px-2 py-1 -ml-1 mt-2 ${open ? "" : "hidden"}`}>
                    {
                        options.map((choice, idx) => <li onClick={() => { setFilter(choice); setOpen(false) }} key={idx} className={`text-sm p-1 my-1 rounded cursor-pointer ${filter == choice ? "bg-blue-50 text-blue-500" : "text-slate-400"} hover:bg-blue-50 hover:text-blue-500 lowercase`}>{ choice }</li>)
                    }
                </ul>
            </div>
