 ...
 * @param required @type Boolean - Required or not
 * @param disabled @type Boolean - Disabled or not
 */
const Input = ({ name, type, color = "stone", value, update, size, min, max, required, disabled }) => {
    return ( 
        <div className={`relative group ${size}`}>
            ...
            <input name={name.toLowerCase()} type={type} value={value} min={min ?? ""} max={max ?? ""} onChange={(e) => update(e.target.value)} onPaste={(e) => e.target.type == "password" ? e.preventDefault() : update(e.target.value)} className={`rounded border group-focus-within:border-${color}-500 w-full p-2 pl-3 focus:outline-none text-sm ${disabled ? "bg-slate-100" : ""}`} autoComplete="off" required={required} disabled={disabled}/>
        </div>
    );
}
